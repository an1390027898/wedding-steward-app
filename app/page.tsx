...
type Wedding = {
  ...
  customs: string[]; times: Record<string, string>; schedule: ScheduleItem[]; contacts: { role: string; name: string; phone: string }[];
};
...
const defaultContacts = ["婚礼管家","男方总管","女方总管","主持人","摄影","摄像","男方化妆师","女方化妆师","车队负责人","酒店负责人"];
...
const blank: Wedding = {
  ...
  times: {}, schedule: [], contacts: defaultContacts.map(role => ({ role, name: role === "婚礼管家" ? "安宁" : "", phone: "" })),
};
...
function makeSchedule(w: Wedding): ScheduleItem[] {
  ...
  const rows = [
    add("gather", "人员集合与物品点验", w.groomAddress || "男方家", "婚礼管家", "大家先不要急着出发。请按清单把花、戒指、红包和车辆逐项给我确认，缺少的现在马上说。", ["手捧花", "婚戒", "分类红包", "车辆表"]),
    ...
    add("tea", "敬茶、改口与拜别", w.brideAddress || "女方家", "婚礼管家", "请父母落座，茶和红包就位。接下来把现场安静下来，把时间留给新人和父母。", ["茶具", "改口红包", "纸巾"]),
  ];
  ...
  rows.push(
    add("groomArrive", "新娘下车进门", w.groomAddress || "男方家", "婚礼管家", "请亲友往两边让，给新人留出完整通道。新人站稳以后再改口、再进门。", ["红伞", "下车红包", "红毯"]),
    add("hotelArrive", "前往酒店与仪式联排", w.hotel || "宴会酒店", "婚礼管家", "到酒店先确认新人、父母和主持人的位置。任何临时增加环节或改时间，请先告诉我。", ["婚戒", "誓言卡", "话筒"]),
    add("ceremony", "婚礼仪式与开席", w.hotel || "宴会酒店", "主持人／婚礼管家", "各岗位按确认后的流程就位。仪式结束后按长辈桌优先顺序敬酒。", ["礼金箱", "敬酒托盘", "备用胸花"]),
  );
  return rows;
}
  ...
  return <main className="app-shell manager">
    ...
    <section className="content">
      ...
      {tab === "board" && current && <section className="page-section board-page">
        ...
        <div className="board-shot-head"><span>赣榆婚礼管家 · 手机当日看板</span><h1>{current.groom} & {current.bride}</h1><p>{current.weddingDate} · 请以管家当天通知为准</p></div>
        <div className="board">
          ...
          {[
            ...
            ["工作人员",current.contacts.map(c=>[c.role,c.name?`${c.name}${c.phone?` · ${c.phone}`:""}`:"待填写"])],
